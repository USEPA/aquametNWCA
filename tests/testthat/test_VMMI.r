...
test_that("VMMI and condition using ECO_X_WETGRP correct",
          {
            ...
            testIn <- subset(testIn, PARAMETER %in% c('FQAI_ALL','N_TOL','RIMP_NATSPP',
                                                      'XRCOV_MONOCOTS_NAT'),
            ...
            testIn.wide <- reshape(testIn, idvar = c('UID','ECO_X_WETGRP'), 
          ...
          testOut.long <- reshape(testOut, idvar = c('UID','ECO_X_WETGRP'), direction = 'long',
...
test_that("VMMI and condition using NWCA_ECO4 and NWCA_WET_GRP correct",
          {
            ...
            testIn.wide <- reshape(testIn, idvar = c('UID','NWCA_ECO4','NWCA_WET_GRP'), 
                                   direction ='wide', timevar = 'PARAMETER',
                                   v.names = 'RESULT') 
            names(testIn.wide) <- gsub("RESULT\\.","", names(testIn.wide))
...
test_that("VMMI lacking ecoregion and wetland type correct",
          ...
          testIn.wide <- reshape(testIn, idvar = c('UID'), 
                                 direction ='wide', timevar = 'PARAMETER',
                                 v.names = 'RESULT') 
          names(testIn.wide) <- gsub("RESULT\\.","", names(testIn.wide))
