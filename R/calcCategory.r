...
calcCategory <- function(vascIn, sampID = "UID") {
  ...
  names(empty_base) <- c(
    "N_DICOT", "N_FERN", "N_GYMNOSPERM", "N_MONOCOT", "N_LYCOPOD", "N_HORSETAIL",
  ...
  names(empty_base.nat) <- c(
    "N_DICOTS_NAT", "PCTN_DICOTS_NAT", "XABCOV_DICOTS_NAT", "XRCOV_DICOTS_NAT",
  ...
  if ("NWCA_NATSTAT" %in% names(vascIn)) {
    ...
    multTraits <- int.combTraits(vascIn.2, c(
      "DICOTS_NAT", "DICOTS_ALIEN", "DICOTS_CRYP", "DICOTS_AC", "FERNS_NAT", "FERNS_INTR", "MONOCOTS_NAT", "MONOCOTS_ALIEN",
      "MONOCOTS_CRYP", "MONOCOTS_AC"
    ), sampID)
