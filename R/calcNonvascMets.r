...
calcNonvascMets <- function(dataIn, nPlot, sampID = "UID", survyear = "2011") {
  ...
  indf1.sum <- aggregate(
    x = list(XCOV = nvstrat.sub$RESULT),
  ...
  if (nrow(subset(nvstrat, PARAMETER == "PEAT_MOSS" & RESULT != "N" & !is.na(RESULT))) > 0) {
    ...
    indf2 <- aggregate(
      x = list(N_PEAT_MOSS_DOM = nvstrat.peat$PLOT),
  ...
  if (survyear == "2011") {
    names(empty_nv) <- c(
      "FREQ_ALGAE", "FREQ_ARBOREAL", "FREQ_BRYOPHYTES", "FREQ_LICHENS",
    ...
    names(empty_nv) <- c(
      "FREQ_ALGAE", "FREQ_BRYOPHYTES", "FREQ_LICHENS", "FREQ_MACROALGAE", "IMP_ALGAE",
