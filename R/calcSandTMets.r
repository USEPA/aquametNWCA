...
calcSandTMets <- function(dataIn, nPlot, sampID = "UID") {
  ...
  vhet1.n <- aggregate(
    x = list(N_SANDT = vhet$SANDT_CLASS),
    by = vhet[c("SAMPID")], FUN = function(x) {
      length(unique(x))
    }
  )
  ...
  vhet4a <- aggregate(
    x = list(DOM_SANDT = vhet4$SANDT_CLASS),
  ...
  vhet5.d <- aggregate(
    x = list(D_SANDT = vhet2$FREQ), by = vhet2[c("SAMPID", "NPLOTS")],
    FUN = function(x) {
      round(1 - sum(x * x), 4)
    }
  )
  ...
  vhet5.h <- aggregate(
    x = list(H_SANDT = vhet2$FREQ), by = vhet2[c("SAMPID", "NPLOTS")],
    FUN = function(x) {
      round(-1 * sum(x * log(x)), 4)
    }
  )
  # Calculate first step in Pielou's Evenness (J)
  vhet5.jcalc <- aggregate(
    x = list(uniqST = vhet2$N_SANDT), by = vhet2[c("SAMPID", "NPLOTS")],
    FUN = function(x) {
      unique(x)
    }
  )
