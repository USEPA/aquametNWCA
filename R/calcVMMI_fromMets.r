...
calcVMMI_fromMets <- function(metsIn, sampID = "UID") {
  ...
  metTholds <- data.frame(
    PARAMETER = c("FQAI_ALL", "N_TOL", "RIMP_NATSPP", "XRCOV_MONOCOTS_NAT"),
  ...
  mmi.1 <- aggregate(
    x = list(VMMI = scored.mets$RESULT), by = scored.mets[keyVars],
    FUN = sum
  )
  ...
  mmiTholds <- data.frame(
    ECO_X_WETGRP = c(
      "CPL-PRLH", "CPL-PRLW", "ALL-EH", "ALL-EW", "EMU-PRLH", "EMU-PRLW",
