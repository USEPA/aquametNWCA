...
calcDuration <- function(vascIn,sampID='UID'){
 # First check for necessary variables
  ...
  durOut.1 <- reshape2::dcast(durOut,as.formula(paste(paste(sampID,collapse='+'),'~PARAMETER',sep=''))
                                 ,value.var='RESULT') %>%
...
calcGrowthHabit <- function(vascIn,sampID='UID'){
  ...
  grhOut.1 <- reshape2::dcast(grhOut,as.formula(paste(paste(sampID,collapse='+'),'~PARAMETER',sep=''))
                              ,value.var='RESULT') %>%
    merge(empty_base, all=TRUE) %>%
    reshape2::melt(id.vars=c(sampID), variable.name='PARAMETER', value.name='RESULT') %>%
    dplyr::filter(!is.na(eval(as.name(sampID[1])))) %>%
...
calcCategory <- function(vascIn,sampID='UID'){
  ...
  catOut.1 <- reshape2::dcast(catOut,as.formula(paste(paste(sampID,collapse='+'),'~PARAMETER',sep=''))
                                 ,value.var='RESULT') %>%
    merge(empty_base, all=TRUE) %>%
    reshape2::melt(id.vars=c(sampID), variable.name='PARAMETER', value.name='RESULT') %>%
    dplyr::filter(!is.na(eval(as.name(sampID[1])))) %>%
...
calcWIS <- function(vascIn,sampID='UID'){
  ...
  wisOut.1 <- reshape2::dcast(wisOut,as.formula(paste(paste(sampID,collapse='+'),'~PARAMETER',sep=''))
                              ,value.var='RESULT') %>%
    merge(empty_base, all=TRUE) %>%
    reshape2::melt(id.vars=c(sampID), variable.name='PARAMETER', value.name='RESULT') %>%
    dplyr::filter(!is.na(eval(as.name(sampID[1])))) %>%
...
calcNative <- function(vascIn,sampID='UID'){
  ...
  natstatOut.1 <- reshape2::dcast(natstatOut,as.formula(paste(paste(sampID,collapse='+'),'~PARAMETER',sep=''))
                                  ,value.var='RESULT') %>%
    merge(empty_base, all=TRUE) %>%
    reshape2::melt(id.vars=c(sampID), variable.name='PARAMETER', value.name='RESULT') %>%
    dplyr::filter(!is.na(eval(as.name(sampID[1])))) %>%
