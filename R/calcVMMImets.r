...
calcVMMImets <- function(vascIn, sampID = "UID") {
  ...
  numtaxa <- aggregate(
    x = list(NUMTAXA = vascIn.fqai$TAXON), by = vascIn.fqai[c("SAMPID")],
    FUN = function(x) {
      length(unique(x))
    }
  )
  ...
  fqaiOut <- aggregate(
    x = list(FQAI_ALL = vascIn.fqai$NWCA_CC),
  ...
  if (nrow(vascIn.nat) > 0) {
    vascIn.nat.1 <- aggregate(
      x = list(XRCOV = vascIn.nat$sXRCOV, RFREQ = vascIn.nat$sRFREQ),
  ...
  if (nrow(vascIn.mono) > 0) {
    vascIn.mono.1 <- aggregate(
      x = list(XRCOV_MONOCOTS_NAT = vascIn.mono$sXRCOV),
