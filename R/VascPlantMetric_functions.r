...
int.calcRich <- function(x, y, tlevel, sampID) {
  ...
  yy3 <- aggregate(
    x = list(XN_TAXA = yy2$N_TAXA), by = yy2[c(sampID, "TOTN_TAXA")],
    FUN = function(z) {
      round(mean(z), 2)
    }
  )
  ...
  yy5 <- aggregate(
    x = list(SDN_TAXA = yy2$N_TAXA), by = yy2[c(sampID, "TOTN_TAXA")],
    FUN = function(z) {
      round(sd(z), 2)
    }
  )
...
int.calcTraits_MultCat <- function(vascIn, trait, sampID) {
  ...
  vascIn.length <- aggregate(
    x = list(N = vascIn1$TAXON), by = vascIn1[c(sampID, trait)],
    FUN = length
  )
  ...
  vascIn1.pctn <- aggregate(
    x = list(uniqN = vascIn1a$TOTN), by = vascIn1a[c(sampID, trait, "N")],
    FUN = unique
  )
  ...
  vascIn.sum <- aggregate(
    x = list(XABCOV = vascIn1$XABCOV, XRCOV = vascIn1$sXRCOV),
...
int.calcTraits_MultCat.alt <- function(vascIn, trait, sampID) {
  ...
  vascIn1 <- subset(vascIn, !is.na(eval(as.name(trait))) & eval(as.name(trait)) != "")
  # calculate number of taxa with trait
  vascIn1.ntax <- aggregate(
    x = list(NTAX = vascIn1$TAXON), by = vascIn1[c(sampID, trait)],
    FUN = length
  )
  ...
  vascIn1.pctn <- aggregate(
    x = list(uniqN = vascIn1a$TOTN), by = vascIn1a[c(sampID, trait, "NTAX")],
    FUN = unique
  )
  ...
  vascIn1.sum <- aggregate(
    x = list(
      XABCOV = vascIn1$XABCOV, XRCOV = vascIn1$sXRCOV,
      RFREQ = vascIn1$sRFREQ
    ), by = vascIn1[c(sampID, trait)],
    FUN = function(z) {
      round(sum(z), 2)
    }
...
int.calcTraits_Indicator <- function(vascIn, trait, sampID) {
  ...
  for (i in 1:length(sampID)) {
    if (i == 1) {
      vascIn$SAMPID <- vascIn[, sampID[i]]
    } else {
      vascIn$SAMPID <- paste(vascIn$SAMPID, vascIn[, sampID[i]], sep = ".")
    }
  }
  ...
  if (nrow(vascIn1) > 0) {
    # calculate number of taxa with trait
    vascIn.length <- aggregate(
      x = list(N = vascIn1$TAXON), by = vascIn1[c("SAMPID")],
      FUN = length
    )
    # Merge back to input data
    ...
    vascIn1.pctn <- aggregate(
      x = list(uniqN = vascIn1a$TOTN), by = vascIn1a[c("SAMPID", "N")],
      FUN = unique
    )
    ...
    outdf1 <- data.frame(
      SAMPID = rep(unique(vascIn$SAMPID), 4),
      PARAMETER = c(
        rep("N", numUIDs), rep("PCTN", numUIDs), rep("XABCOV", numUIDs),
        rep("XRCOV", numUIDs)
      ), RESULT = 0, stringsAsFactors = F
    )
...
int.calcTraits_Indicator.alt <- function(vascIn, trait, sampID) {
  ...
  # Calculate number of taxa with trait
  vascIn1.ntax <- aggregate(
    x = list(NTAX = vascIn1$TAXON), by = vascIn1[c("SAMPID")],
    FUN = length
  )
  ...
  vascIn1.pctn <- aggregate(
    x = list(uniqN = vascIn1a$TOTN), by = vascIn1a[c("SAMPID", "NTAX")],
    FUN = unique
  )
...
int.calcIndices <- function(vascIn, subgrp = NULL, sampID) {
  ...
  vascIn.sum <- aggregate(
    x = list(SUBXTOTABCOV = vascIn$XABCOV),
  ...
  vascIn.1.sum <- aggregate(
    x = list(Hsub = vascIn.1$hcalc, Dsub = vascIn.1$dcalc),
    by = vascIn.1[c(sampID)],
    FUN = sum
  )
  # Count number of taxa
  vascIn.1.jcalc <- aggregate(
    x = list(jcalc = vascIn.1$TAXON),
    by = vascIn.1[c(sampID)],
    FUN = length
  )
...
int.calcRichNS <- function(x, y, natstat, grpname, sampID) {
  ...
  xx1 <- aggregate(
    x = list(TOTN_TAXA = xin$DISTINCT), by = xin[c(sampID)],
    FUN = sum
  )
  ...
  yy1 <- merge(subset(y, NWCA_NATSTAT %in% natstat,
    select = c(sampID, "PLOT", "DISTINCT")
  ), xx1, by = sampID)
  # Calculate number of taxa by plot
  yy2 <- aggregate(
    x = list(N_TAXA = yy1$DISTINCT),
...
int.calcXBC <- function(x, sampID) {
  ...
  for (i in 1:length(sampID)) {
    if (i == 1) {
      ...
      x$SAMPID <- paste(x$SAMPID, x[, sampID[i]], sep = ".")
    }
  }
