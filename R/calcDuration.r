...
calcDuration <- function(vascIn, sampID = "UID") {
  ...
  if ("DUR_ALT" %in% names(vascIn)) {
    ...
    vascIn.1$DUR_ALT[vascIn.1$DURATION %in% c(
      "ANNUAL, BIENNIAL",
      "BIENNIAL",
      "BIENNIAL, AN"
    )] <- "ANN_BIEN"
  ...
  names(empty_base) <- c(
    "N_ANN_BIEN", "N_ANN_PEREN", "N_ANNUAL", "N_PERENNIAL", "PCTN_ANN_BIEN", "PCTN_ANNUAL",
  ...
  names(empty_base.nat) <- c(
    "N_ANN_BIEN_AC", "N_ANN_BIEN_NAT", "N_ANN_PEREN_AC", "N_ANN_PEREN_NAT",
  ...
  if ("NWCA_NATSTAT" %in% names(vascIn.1)) {
    ...
    multTraits <- int.combTraits(vascIn.2, c(
      "ANNUAL_NAT", "ANNUAL_AC", "ANN_BIEN_NAT", "ANN_BIEN_AC", "ANN_PEREN_NAT",
      "ANN_PEREN_AC", "PERENNIAL_NAT", "PERENNIAL_AC"
    ), sampID)
