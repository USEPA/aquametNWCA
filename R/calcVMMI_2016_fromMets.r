...
calcVMMI_2016_fromMets <- function(metsIn, sampID='UID', wetcls_grp=NULL){
  ...
  if(sum(necVars %in% names(metsIn))<2){
    ...
    necMets <- data.frame(wet_grp = c(rep('EH', 6), rep('EW', 6), rep('PRLH', 4), rep('PRLW', 4)),
                          METRIC = c('N_ANNUAL', 'PCTN_NATSPP', 'XRCOV_FORB', 'XRCOV_HTOL',
  ...
  if(sum(necMets_sub$METRIC %in% names(metsIn)) < nrow(necMets_sub)){
    ...
    print(paste(paste(msgMets, collapse = ', '), 
  ...
  metTholds <- data.frame(wetcls_grp = c(rep('EH', 6), rep('EW', 6),
                                         rep('PRLH', 4), rep('PRLW', 4)),
                          PARAMETER=c('N_ANNUAL', 'PCTN_NATSPP', 'XRCOV_FORB',
  ...
  vMet <- merge(metsIn.long, metTholds, by.x = c(wetcls_grp, 'PARAMETER'), 
  ...
  mmi.1 <- aggregate(x = list(VMMI_2016 = scored.mets$RESULT), by = scored.mets[keyVars],
                     FUN = sum)
  
  numMets <- data.frame(wetcls_grp = c('EH', 'EW', 'PRLH', 'PRLW'), 
  ...
  mmiTholds <- data.frame(RPT_FINAL=c('EH', 'EW', 'PRLH', 'PRLW-OTHER', 'PRLW-PLN_ARIDW'),
