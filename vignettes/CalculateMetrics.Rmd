...
```{r allVasc.2}
outdf <- calcVascPlantMets(vascIn=VascPlantEx,taxaIn=taxaNWCA,taxaNat=ccNatNWCA, taxaCC=ccNatNWCA,taxaWIS=wisNWCA,sampID='UID',cValReg='STATE')
...
```{r subVasc.1}
sumdf <- nwcaVegData(vascIn=VascPlantEx, sampID='UID', inTaxa=taxaNWCA, inNat=ccNatNWCA, inCVal=ccNatNWCA, inWIS=wisNWCA, cValReg='STATE')
str(sumdf)
```
...
```{r subVasc.2}
outRich <- calcRichness(sumdf$byUIDspp,sumdf$byPlotspp,sumdf$byUIDgen,sumdf$byPlotgen,
                        sumdf$byUIDfam,sumdf$byPlotfam, 
                        sampID='UID')
head(outRich)
...
outVasc <- reshape(allVasc, idvar = 'UID', direction = 'wide',
...
nplots <- aggregate(x = list(NPLOTS = TreesEx$PLOT),
                    by = TreesEx[c('UID')], 
                    FUN = function(x){length(unique(x))})

outTree <- calcTreeMets(TreesEx, nPlotIn=nplots, sampID='UID')
head(outTree)
```
...
```{r subTree.1}
outSnag <- calcSnagMets(TreesEx, nPlot=nplots, sampID='UID')
head(outSnag)
...
nplots <- aggregate(x = list(NPLOTS = subVtype$PLOT), 
                    by = subVtype[c('UID')],
                    FUN = function(x){length(unique(x))})
...
outAllVG.wide <- reshape(outAllVG, idvar = 'UID', direction = 'wide',
                         timevar = 'PARAMETER', v.names = 'RESULT')
